...
if [ ${MODEL} = 7b-chat ]; then
	...
	torchrun --nproc_per_node 2 chat_test.py --ckpt_dir llama-2-13b-chat/ --tokenizer_path tokenizer.model --max_seq_len ${LEN} --max_batch_size ${BAT}
elif [ ${MODEL} = 30b-chat ]; then
	torchrun --nproc_per_node 8 chat_test.py --ckpt_dir llama-2-30b-chat/ --tokenizer_path tokenizer.model --max_seq_len ${LEN} --max_batch_size ${BAT}
else
